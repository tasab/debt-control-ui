...
/**
 * Те саме, але без копійок: "1 000 501" → "1 001".
 *
 * Для перерахунку кас копійки — шум: суми там у тисячах, і два зайві знаки в
 * кожному рядку лише заважають зіставляти цифри поглядом. Округлення тут суто
 * для показу; у журнал іде рівно те, що ввели.
 */
export function formatWhole(value, { exponent = DEFAULT_EXPONENT, locale = 'uk-UA', sign = 'auto' } = {}) {
  const raw = normalise(value)
  const negative = raw.startsWith('-')
  const scale = 10n ** BigInt(exponent)
  const abs = BigInt(negative ? raw.slice(1) : raw)
  // Половина й більше — вгору, як і очікує людина від «приблизно».
  const rounded = (abs + scale / 2n) / scale

  const body = new Intl.NumberFormat(locale).format(rounded)
  if (negative) return `−${body}`
  if (sign === 'always' && rounded > 0n) return `+${body}`
  return body
}
...
/**
 * Difference between two amounts of the same currency. Used for the
 * «порахував − облік» preview at closing time, so the difference is visible
 * before submitting rather than only in the toast afterwards. The server
 * recomputes it under a lock and its answer is the one that gets posted.
 */
export const subtractAmounts = (a, b) => (BigInt(normalise(a)) - BigInt(normalise(b))).toString()
